...
if __name__ == "__main__":
    ...
    with mlflow.start_run():
        ...
        rmse_2, mae_2, r2_2 = eval_metrics(test_y, y_pred_dt)
        
        # Output the metrics to the stdout   
        # (this is somewhat equivalent to print)
        sys.stdout.write("  Decision Tree Model:\n")
        sys.stdout.write("  RMSE: %s\n" % rmse_2)
        sys.stdout.write("  MAE: %s\n" % mae_2)
        sys.stdout.write("  R2: %s\n\n" % r2_2)
        
        # Log the metrics to mlflow
        mlflow.log_metric("Decision_rmse", rmse_2) # root mean square error
        mlflow.log_metric("Decision_r2", r2_2) # r squared
        mlflow.log_metric("Decision_mae", mae_2) # mean absolute error
        ...
        if tracking_url_type_store != "file":            # Register the model
            ...
            mlflow.sklearn.log_model(dtModel, "model", registered_model_name="Decision Tree Micronutrient Model")
        else:
            mlflow.sklearn.log_model(dtModel, "model")
